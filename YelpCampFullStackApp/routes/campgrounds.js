...
router.post("/", middleware.isLoggedIn, function(req, res){
    ...
    //create new campground and save into DB
    Campground.create(newCampground, function(err, newlyCreated){
       if(err){
           console.log(err);
       } 
       else {
           console.log("no error when adding campground!");
...
router.get("/:id", function(req, res){
    ...
    Campground.findById(req.params.id).populate("comments").exec(function(err, foundCampground){
       //populates the comment array with the actual comments themselves instead of just IDS
       if(err){
           console.log(err);
       } 
       else{
...
router.delete("/:id", middleware.checkCampgroundOwnership, function(req, res){
    //find campground to delete and then delete it from mongodb
